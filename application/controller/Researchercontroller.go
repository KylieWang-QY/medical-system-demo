...
func Researcher(ctx *gin.Context) {
	ctx.HTML(http.StatusOK, "researcher.html", "")
}

func ResearcherQuery(ctx *gin.Context) {
	ctx.HTML(http.StatusOK, "researcher_query_cases.html", "")
}

func ToResearcherQuery(ctx *gin.Context) {
	form := struct {
	...
	args := [][]byte{[]byte("researcher"), []byte(form.Username), []byte(form.KeyWord), []byte(utils.GetTime())}
